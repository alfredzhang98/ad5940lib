...
FreqParams_Type AD5940_GetFreqParameters(float freq)
{
  ...
  const uint32_t sinc3osr_table[] = {2, 4, 5};
	...
	uint32_t iCycle = 0;
	FreqParams_Type freq_params = {0, 0, 0, 0, 0, 0};
	/* High power mode */
	if(freq >= 20000)
	{
...
AD5940Err AD5940_HSRtiaCal(HSRTIACal_Type *pCalCfg, void *pResult)
{
  ...
  *******************************/
  ...
  uint64_t const HSTIADERLOADTable[]={0,10,30,50,100,999999999999};
  uint64_t const HSTIADERTIATable[] = {50,100,200,1000,5000,10000,20000,40000,80000,160000,0,999999999999999};
  uint32_t WgAmpWord;
